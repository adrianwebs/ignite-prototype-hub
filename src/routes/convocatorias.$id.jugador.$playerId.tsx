...
export default function Page() {
  ...
  const data = sess.map((s, i) => {
    const r = records.find((x) => x.sessionId === s.id && x.playerId === player.id);
    ...
    const uaWindow = (ws: typeof sess) =>
      ws.map((ww) => {
        const rr = records.find((x) => x.sessionId === ww.id && x.playerId === player.id);
  ...
  const alerts = data.filter((d) => d.ua > moderado);
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-muted/40 text-xs text-muted-foreground">
                <tr>
                  {["Fecha", "Sesión", "Tipo", "Dur", "RPE", "Fatiga", "UA", "A:C", "Estado"].map(
                    (h) => (
                  ...
                  return (
                    <tr key={d.id} className="border-t hover:bg-muted/20">
                      ...
                      <td
                        ...
                        style={{ color: statusColor(acStatus(d.ac)) }}
                      >
                        {d.ac.toFixed(2)}
                      </td>
                      <td className="px-3 py-2">
